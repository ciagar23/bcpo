...
function doLogin()
{
	...
	if ($userName == '') {
		...
		$sql = "SELECT w_id
		        FROM tbl_webuser 
				WHERE w_username = '$userName' AND w_password = PASSWORD('$password')";
		$result = dbQuery($sql);
	
		if (dbNumRows($result) == 1) {
			$row = dbFetchAssoc($result);
			$_SESSION['website_id'] = $row['w_id'];
			
			// log the time when the user last login
			$sql = "UPDATE tbl_webuser 
			        SET w_logindate = NOW() 
					WHERE w_id = '{$row['w_id']}'";
			dbQuery($sql);
...
function CrimeListOption($crime)
{
	$sql = "SELECT c_id, c_parent_id, c_name
			FROM tbl_crime
			ORDER BY c_id";
	$result = dbQuery($sql) or die('Cannot get Crime. ' . mysql_error());
