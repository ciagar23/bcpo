...
$session = $_SESSION["website_id"];	
$sql1 = "SELECT w_username FROM tbl_webuser where w_id ='$session' ";
	...
	$sql   = "INSERT INTO tbl_reportacrime (r_from, r_complaint, r_report, r_read, r_to, r_status, r_datetime)
	          VALUES ('$username','$complaint', '$report', 'no', '$to', 'reply', NOW())";
...
$result1 = mysql_query($sq1l) or die('Cannot get records. ' . mysql_error());
$row1 = mysql_fetch_assoc($result1);
extract($row1);
	
    $action       = $_POST['action'];
	$sql   = "INSERT INTO tbl_reportacrime (r_from, r_complaint, r_report, r_read, r_to, r_status, r_datetime)
	          VALUES ('$to','$r_complaint', '$r_report', 'no', '$action', 'action', NOW())";
...
$sql1 = "SELECT w_username FROM tbl_webuser where w_id ='$session' ";
    ...
    $complaint       = $_POST['txtcomplaint'];
    $incidentdate       = $_POST['txtincidentdate'];
    $incidentplace       = $_POST['txtincidentplace'];
    $report       = $_POST['txtreport'];
	$sql   = "INSERT INTO tbl_reportacrime (r_from, r_complaint, r_report,r_incidentdate, r_incidentplace, r_read, r_to, r_status, r_datetime)
	          VALUES ('$username','$complaint', '$report', '$incidentdate', '$incidentplace', 'no', 'Operations', 'report', NOW())";
...
bl_rinjury, bl_brgy, bl_prosec, bl_court, bl_dtg)
	...
	$sql   = "INSERT INTO tbl_history (h_date, h_user, h_action, h_station, h_link)
	          VALUES (NOW(), '$blottedby', 'Recase Blotter no. $bl_id', '$station','/bcpo/admin/blotter/index.php?view=detail&policeId=$policeId')";
	dbQuery($sql);
