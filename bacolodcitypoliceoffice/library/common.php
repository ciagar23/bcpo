...
function getPagingLink($sql, $itemPerPage = 10, $strGet = '')
{
	...
	if ($totalPages > 1) {
		...
		if ($pageNumber > 1) {
			$page = $pageNumber - 1;
			if ($page > 1) {
				$prev = " <a href=\"$self?page=$page&view=$view/\">[Prev]</a> ";
			} else {
				$prev = " <a href=\"$self?view=$view\">[Prev]</a> ";
			}	
				
			$first = " <a href=\"$self?view=$view\">[First]</a> ";
		} else {
		...
		if ($pageNumber < $totalPages) {
			$page = $pageNumber + 1;
			$next = " <a href=\"$self?page=$page&view=$view\">[Next]</a> ";
			$last = " <a href=\"$self?page=$totalPages&view=$view\">[Last]</a> ";
		} else {
		...
		for($page = $start; $page <= $end; $page++)	{
			if ($page == $pageNumber) {
				...
				if ($page == 1) {
					$pagingLink[] = " <a href=\"$self?view=$view\">$page</a> ";
				} else {	
					$pagingLink[] = " <a href=\"$self?page=$page&view=$view\">$page</a> ";
				}	
			}
